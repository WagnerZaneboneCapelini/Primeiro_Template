...
ENV VIRTUAL_ENV /opt/Primeiro_Template-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/Primeiro_Template/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/Primeiro_Template/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/primeiro_template/ && touch src/primeiro_template/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/Primeiro_Template/
